...
void averageNormals(pcl::PointCloud<pcl::Normal>::Ptr cloud_normals) {
    // 检查云是否为空
    ...
    for (size_t i = 0; i < cloud_normals->size(); ++i) {
        ...
        average_normal += Eigen::Vector3f(normal.normal_x, normal.normal_y, normal.normal_z);
    }

    // 计算平均法向量
    average_normal /= cloud_normals->size();

    // 创建一个新的法向量
    pcl::Normal avg_normal;
    avg_normal.normal_x = average_normal[0];
    avg_normal.normal_y = average_normal[1];
    avg_normal.normal_z = average_normal[2];

    cloud_normals->clear();
    cloud_normals->push_back(avg_normal);

    // 输出结果
    //PCL_INFO("Average Normal: (%f, %f, %f)\n", avg_normal.normal_x, avg_normal.normal_y, avg_normal.normal_z);
}

void averagePoint(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_point)
{
    // 检查点云是否为空
    if (cloud_point->empty()) {
        std::cerr << "The point cloud is empty!" << std::endl;
        return;
    }

    // 计算所有点的平均值
    pcl::PointXYZ avg_point(0.0, 0.0, 0.0);  // 用来存储计算出来的平均点
    for (const auto& point : cloud_point->points) {
        avg_point.x += point.x;
        avg_point.y += point.y;
        avg_point.z += point.z;
    }
    ...
    // 对每个坐标分量取平均值
    avg_point.x /= cloud_point->size();
    avg_point.y /= cloud_point->size();
    avg_point.z /= cloud_point->size();

    cloud_point->clear();
    cloud_point->push_back(avg_point);
}
...
//   pcl::PCLPointCloud2 cloud_filtered;
  ...
  pass.setFilterLimits (0.04, 0.06);
  pass.filter (*indicate_cloud);
...
//   ne.setRadiusSearch (0.03);
  ...
  //多个法向量平均值
  averageNormals(cloud_normals);
  averagePoint(indicate_cloud);
  //合并点云和法向量
